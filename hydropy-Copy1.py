...
class hydro(threading.Thread):
    def __init__(self,dt,X,A,rho,rhou,rhoe,hrate,gamma=5./3,maxiter=1e3,each=0):
        ...
        self.dt = dt
        self.rho = rho*self.A
        self.rhou = rhou*self.A
        self.rhoe = rhoe*self.A
        self._hrate = hrate
        self.g = -d(self.X[:,2])/self.ds*Sun.g_sun
        self.gamma = gamma
        self.kappa = Sun.kappa*self.A
        self.k_b = Sun.k_b
    ...
    def hrate(self,time):
        return self._hrate(time)*self.A
    ...
    def radloss(self, T):
        return np.interp(T, self.rlTemperature, self.rlRate, left = 0, right = 0)/self.A
    ...
    def diffuse(self):
        ...
        dd = self.kappa*self.T**2.5*self.m/(3*self.k_b)/self.rho
    ...
    def _boundary(self):
        ...
        self.rho[:2] = self.p[:2]/(2*self.k_b/self.m*self.T[:2])
        self.rho[-2:] = self.p[-2:]/(2*self.k_b/self.m*self.T[-2:])
    ...
    def boundary(self):
        ...
        self.rho[0] = self.p[0]/(2*self.k_b/self.m*self.T[0])
        self.rho[-1] = self.p[-1]/(2*self.k_b/self.m*self.T[-1])
    ...
    def getptu(self):
        ...
        self.T = (self.eth*self.m/(3*self.k_b))
    ...
    def run(self):     
        ...
        for i in range(0,self.maxiter):
            ...
            if (self.each != 0):
                if (i % self.each == 0):
                    I1 = (self.rho/self.A/self.m)**2*AIA.t171(self.T)*1e8
                    I2 = (self.rho/self.A/self.m)**2*AIA.t193(self.T)*1e8
                    I3 = (self.rho/self.A/self.m)**2*AIA.t211(self.T)*1e8
                    ...
                    plt.plot(self.s, np.log10(self.rho/self.A/self.m)-4)
